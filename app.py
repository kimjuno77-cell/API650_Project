...
from Visualization import generate_shell_svg, generate_nozzle_orientation_svg, generate_wind_moment_svg, generate_roof_detail_svg, generate_seismic_spectrum_graph
...
# Auto Load Session State (Prevents data loss on refresh)
...
with st.container():
    ...
    with col_geom:
        ...
        if conical_bottom_type != "Flat":
            ...
            slope_conical_bottom = 0.0
            
        D = ID_input / 1000.0  # Convert to meters for internal calc
        H = H_input / 1000.0
        # Map for legacy code
        max_level = HD / 1000.0
        min_level = HLL / 1000.0

    with col_cap:
        import math
        # Dynamic calculations for capacities
        Vn = (math.pi * (D**2) / 4) * (H_input / 1000.0)
        Vop = (math.pi * (D**2) / 4) * (HHL / 1000.0)
        Vnet = (math.pi * (D**2) / 4) * ((HHL - HLL) / 1000.0)
        
        st.info(f"**Calculated Nominal Capacity (Vn):** {Vn:.1f} m³")
        st.info(f"**Calculated Storage Capacity (Vop):** {Vop:.1f} m³")
        st.info(f"**Calculated Net Working Capacity (Vnet):** {Vnet:.1f} m³")
    ...
    st.markdown('---')
    st.subheader('AUTOMATIC ANNEX DETECTION (API 650)')
    applicable_annexes = []
    if operating_temp > 93:
        applicable_annexes.append('Annex M (High Temperature)')
    if P_external_mm > 0:
        applicable_annexes.append('Annex V (External Pressure)')
    if P_design_mm > 0:
        applicable_annexes.append('Annex F (Internal Pressure)')
    if 'EFRT' in tank_roof_type:
        applicable_annexes.append('Annex C (External Floating Roof)')
    if 'IFRT' in tank_roof_type:
        applicable_annexes.append('Annex H (Internal Floating Roof)')
        
    if applicable_annexes:
        st.success('**Applicable Annexes:** ' + ', '.join(applicable_annexes))
    else:
        st.info('**Applicable Annexes:** None (Basic Appendix A/Base Design)')
    ...
    with st.expander("Advanced Roof Settings", expanded=False):
        ...
        detail_type = "Detail a (Top Angle)"
        top_angle_size = "L75x75x6"
        
    st.header("2. MATERIAL PROPERTIES & CORROSION ALLOWANCE")
...
# --- Tab 2: Loads ---
with st.container():
    st.header("2. WIND & SEISMIC DESIGN DATA")
...
with st.container():
    ...
    if roof_design and hasattr(roof_design, 'results'):
        r_res = roof_design.results
        # 1. Critical Actions (e.g. Type Change)
        if r_res.get('Action'):
            st.error(f"🚨 **CRITICAL DESIGN ALERT**: {r_res.get('Error', 'Design Constraints Failed')}")
            st.warning(f"🛠️ **Automatic Action Taken**: {r_res.get('Action')}")
            if r_res.get('Note'):
                st.info(f"ℹ️ **Reason**: {r_res.get('Note')}")
            st.divider()
            
        # 2. Warnings (e.g. Slope Adjustment)
        elif r_res.get('Warning'):
            # Check if it's a slope adjustment warning (Yellow)
            if "Slope automatically increased" in r_res.get('Warning', ''):
                st.warning(f"⚠️ **Design Adjusted**: {r_res.get('Warning')}")
                if r_res.get('Note'):
                    st.caption(f"Reason: {r_res.get('Note')}")
                st.divider()
            else:
                # Generic Warning
                st.warning(f"⚠️ Warning: {r_res.get('Warning')}")
                st.divider()
    if api_warnings:
    ...
    if 'use_kds' in locals() and use_kds:
        ...
        # Use governing or API params?
        # Graph should probably show API 650 Spectrum as primary.
        p_sds = params.get('SDS', 0.0)
        p_sd1 = params.get('SD1', 0.0)
        p_tl = params.get('TL', 4.0)
        
        # Call Visualization Function
        title_graph = "Design Response Spectrum (API 650)"
        seismic_graph_b64 = generate_seismic_spectrum_graph(p_sds, p_sd1, p_tl, title_graph)
        
        if seismic_graph_b64:
             st.image(f"data:image/png;base64,{seismic_graph_b64}")
        else:
             st.caption("Graph not available (SDS or SD1 is zero)")

        # KDS Comparison Plot (Optional - could overlay if implemented in Visualization, but for now simple)
        if 'use_kds' in locals() and use_kds:
             st.caption("Note: KDS Spectrum overlay not yet supported in this view.")
    ...
    report_data = {
        ...
        'results': {
            ...
            'top_member': locals().get('top_member_res', {}),
    ...
    if 'report_data' not in st.session_state:
        ...
        if "Ver.2026" in report_type:
            ...
            html_gen.set_results(
                ...
                seismic_graph=extended_context.get('seismic_graph'), # Changed from rd.get('seismic_graph', "")
                anchor=rd.get('anchor', {}),
                annex_f=res.get('annex_f_res', {}),
                anchor_chair=rd.get('anchor_chair', {}), # Changed from res.get('anchor_chair_res', {})
                standard_comparison=rd.get('standard_comparison', {}),
                gov_codes=rd.get('gov_codes', {}),
                seismic_hoop_res=res.get('seismic_hoop_res', {}),
...
# --- AUTO SAVE SESSION ---
try:
    with open(".last_session.json", "w", encoding="utf-8") as f_out:
        f_out.write(save_project_to_json())
except:
    pass
