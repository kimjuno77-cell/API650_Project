...
with tab4:
    ...
    if 'report_data' not in st.session_state:
        ...
        # --- STALE DATA CHECK ---
        # Check if new keys exist. If not, force re-run.
        ...
        # Prepare unpacked dictionaries for set_results
        ...
        # Only P_wind_kPa...
        # I must reconstruct wind_data here if missing.
        
        wind_data_in = res.get('wind_res', {})
        if not wind_data_in:
             # Reconstruct minimal if missing
             p_w_kpa = rd['design_data'].get('P_design', 0) / 100 # Rough guess or use inputs
        ...
        html_gen.set_results(
            ...
            anchor_data=rd['anchor'], # Detailed Results including Chair Dims
            ...
            seismic_data={
                **res['seismic_res'],
                ...
                'Ms_kNm': res['seismic_res'].get('Slab_Moment_kNm', 0), # SAFE KEY
                ...
                'Importance_Factor': I_seismic if 'I_seismic' in locals() else res['seismic_res'].get('Importance Factor', 1.0) # SAFE KEY
            },
            ...
            nozzle_data=res.get('nozzle_res', []),
            efrt_data=res.get('efrt_res', {}),
            standard_comparison=html_gen.data_context.get('standard_comparison', {}),
            frangibility=res.get('frangibility_res', {})
        )
        
        # Inject Extended Data
        if 'extended' in rd:
            html_gen.data_context.update(rd['extended'])
        
        # FORCE UPDATE with Fresh SVGs (Fallback Check)
        # This ensures that even if rd['extended'] had empty strings, the locally regenerated ones are used.
        html_gen.data_context.update({
            'shell_svg': shell_svg,
            'nozzle_svg': nozzle_svg,
