...
class RoofDesign:
    ...
    def check_roof_plate(self, total_load_kPa=1.2):
        ...
        if 'self-supported cone' in self.roof_type.lower():
            ...
            # Iterative check for Slope / Thickness
            max_slope_deg = 30.0
            current_slope = self.slope
            
            # Helper to calc t_min for a given slope
            def calc_t_min(s):
                th = math.atan(s)
                sin_th = math.sin(th)
                if sin_th > 0.001:
                    return self.D / (4.8 * sin_th)
                return 999.0
            
            t_geom_mm = calc_t_min(current_slope)
            t_min_std = max(5.0, t_geom_mm)
            
            # If thickness exceeds 13mm, try increasing slope
            if t_min_std > 13.0:
                self.results['Note'] = "Initial Self-Supported design exceeded 13mm. Attempting slope adjustment..."
                
                # Iterate slope from current up to max_slope_deg
                # Step 0.5 degrees
                # import math (removed to avoid local scope error)
                curr_deg = math.degrees(math.atan(current_slope))
                found_valid_slope = False
                
                while curr_deg <= max_slope_deg:
                    curr_deg += 0.5
                    new_slope = math.tan(math.radians(curr_deg))
                    t_check = calc_t_min(new_slope)
                    t_std_check = max(5.0, t_check)
                    
                    if t_std_check <= 13.0:
                        self.slope = new_slope
                        t_min_std = t_std_check
                        found_valid_slope = True
                        self.results['Warning'] = f"Slope automatically increased to {curr_deg:.1f} deg ({self.slope:.4f}) to satisfy 13mm limit."
                        break
                
                if not found_valid_slope:
                    # Fallback to Supported Cone Roof
                    self.results['Error'] = "Self-Supported design failed (Req Thk > 13mm even at max slope)."
                    self.results['Action'] = "Switched to Supported Cone Roof."
                    
                    self.roof_type = 'Supported Cone Roof'
                    # Re-run check as Supported Cone Roof by calling recursively or just continuing
                    # If we switch type, we should probably reset/re-evaluate. 
                    # Let's handle the "Supported" logic in the next pass or right here.
                    # Simple way: Change type and let the next block (if supported block exists or is implicit) handle it.
                    # But 'supported' is not explicitly handled in original "if 'self-supported..." block, 
                    # it falls through if not careful.
                    # The original code structure:
                    # if 'self-supported cone': ...
                    # elif 'dome'...: ...
                    # else: (implicit supported?) -> Wait, original code didn't have specific "Supported" block?
