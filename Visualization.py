...
def generate_seismic_spectrum_graph(SDS, SD1, T_L=4.0, title="Design Response Spectrum"):
    """
    Generates a Design Response Spectrum curve (Sa vs T) per ASCE 7 / API 650.
    ...
    try:
        ...
        # 1. Define Period Range (0 to T_L + 2s)
        T_max = max(T_L + 2.0, 6.0)
        periods = np.linspace(0, T_max, 200)
        sa_values = []
        
        # 2. Calculate T0 and Ts
        if SDS > 0:
            Ts = SD1 / SDS
            T0 = 0.2 * Ts
        else:
        ...
        ax.plot(periods, sa_values, label='Design Spectrum', color='blue', linewidth=2)
        ...
        if SDS > 0:
            ax.scatter([T0, Ts], [SDS, SDS], color='red', zorder=5)
            ax.annotate(f'SDS={SDS:.2f}', xy=(Ts, SDS), xytext=(Ts+0.5, SDS+0.05),
                        arrowprops=dict(facecolor='black', shrink=0.05))
            ax.annotate(f'SD1={SD1:.2f}', xy=(1.0, SD1), xytext=(1.5, SD1+0.1),
                        arrowprops=dict(facecolor='black', shrink=0.05))
